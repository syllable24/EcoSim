...
static SDL_Texture* hex_mask = NULL;
...
void draw_hexagon(SDL_Renderer* renderer, SDL_FPoint* vertices) {
    for (int i = 0; i < 6; i++) {
        int next = (i + 1) % 6;
        SDL_RenderLine(
            renderer, 
...
int create_hexagon_texture_mask(){    
    hex_mask = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, HEX_MASK_SIZE, HEX_MASK_SIZE);    
    if (!hex_mask) {
        SDL_Log("Failed to create hex mask: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }

    // Set render target to hex_mask
    SDL_SetRenderTarget(renderer, hex_mask);
    SDL_SetRenderDrawColor(renderer, 0, 0, 0, 0);
    SDL_RenderClear(renderer);

    // Draw filled hexagon
    SDL_FPoint points[6];
    get_hexagon_vertices(points, HEX_MASK_SIZE / 2.0f, HEX_MASK_SIZE / 2.0f, HEX_RADIUS);
    SDL_SetRenderDrawColor(renderer, 255, 255, 255, 255);

    // Approximate filled hexagon by drawing triangles (fan from center)
    SDL_Vertex vertices[7];
    vertices[0].position.x = HEX_MASK_SIZE / 2.0f;
    vertices[0].position.y = HEX_MASK_SIZE / 2.0f;
    ...
    for (int i = 0; i < 6; i++) {
        ...
        vertices[i + 1].color.r = 255;
        vertices[i + 1].color.g = 255;
        vertices[i + 1].color.b = 255;
        vertices[i + 1].color.a = 255;
    }
    ...
    SDL_SetTextureBlendMode(hex_mask, SDL_BLENDMODE_BLEND);
...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[]){
    ...
    // Create Hex texture mask
    if (create_hexagon_texture_mask() != SDL_APP_CONTINUE){
        SDL_Log("Failed to create hex mask: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }

    render_target = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, HEX_MASK_SIZE, HEX_MASK_SIZE);
...
void SDL_AppQuit(void *appstate, SDL_AppResult result){
    ...
    if (hex_mask) SDL_DestroyTexture(hex_mask);
